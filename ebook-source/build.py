...
def divider(num, eyebrow, title, sub, chips, rt):
    ...
    inner = f'''<div class="wrap">
      ...
      <div class="rt">@@IC:clock@@ Lecture&nbsp;: {rt}</div>
...
_BOX = {
 "case":("box--case","@@IC:eye@@","Étude de cas"),
 "error":("box--error","@@IC:xmark@@","Erreur à éviter"),
 "tip":("box--tip","@@IC:sparkle@@","Astuce Cheapsappes"),
 "ex":("box--ex","@@IC:edit@@","Exercice pratique"),
}
...
def process(title, steps):
    """Schéma paysage : les étapes d'un processus, avec icônes et flèches."""
    out=[f'<div class="proc"><div class="proc-t">{title}</div><div class="proc-row">']
    for i,(icon,label) in enumerate(steps):
        if i>0: out.append('<div class="proc-ar">@@IC:arrow@@</div>')
        out.append(f'<div class="proc-st"><div class="proc-ic">{icon}</div>'
                   f'<div class="proc-n">{i+1}</div><div class="proc-l">{label}</div></div>')
    out.append('</div></div>')
    return "".join(out)
...
def vs(title_a, list_a, title_b, list_b, label="VS"):
    ...
    return (f'<div class="vswrap"><div class="divlabel">{label}</div><div class="vs">'
            f'<div class="col col--a"><div class="ch">@@IC:xmark@@ {title_a}</div><ul>{la}</ul></div>'
...
page(
  ...
  '<div class="card" style="border-left:3px solid #1B63F0"><div class="ct">@@IC:eye@@ Étude de cas</div><p>Un parcours réel et chiffré, pour voir la méthode appliquée à une vraie personne.</p></div>'
  '<div class="card" style="border-left:3px solid #E24435"><div class="ct">@@IC:xmark@@ Erreur à éviter</div><p>Le piège classique du débutant sur ce chapitre — et comment le contourner.</p></div>'
  '<div class="card" style="border-left:3px solid #0FB5B0"><div class="ct">@@IC:sparkle@@ Astuce Cheapsappes</div><p>Le raccourci d\'initié&nbsp;: ce que font les revendeurs qui vont vite.</p></div>'
  '<div class="card" style="border-left:3px solid #E39A2B"><div class="ct">@@IC:edit@@ Exercice pratique</div><p>10 minutes, de quoi transformer la lecture en action concrète.</p></div>'
  '</div>' +
...
page(
  ...
  '<div class="card" style="border-left:3px solid #1B63F0"><div class="ct">@@IC:globe@@ Un marché immense</div><p>Des dizaines de millions d\'acheteurs actifs sur la seconde main, une demande qui explose et une offre de qualité toujours insuffisante.</p></div>'
  '<div class="card" style="border-left:3px solid #1B63F0"><div class="ct">@@IC:zap@@ Zéro frais fixes</div><p>Pas de local, pas de stock imposé, pas d\'abonnement. Tu commences avec ce que tu as et tu réinvestis tes gains.</p></div>'
  '<div class="card" style="border-left:3px solid #1B63F0"><div class="ct">@@IC:euro@@ Cash-flow rapide</div><p>Un article publié le dimanche peut être vendu et payé dans la semaine. Le cycle argent → stock → argent est court.</p></div>'
  '<div class="card" style="border-left:3px solid #1B63F0"><div class="ct">@@IC:cap@@ Compétences réelles</div><p>Sourcing, pricing, photo, négociation, gestion de stock&nbsp;: un vrai socle d\'entrepreneur, réutilisable partout.</p></div>'
  '</div>' +
  box("case",
...
page(
  ...
  flow([("@@IC:layers@@","Fondations","lexique + mindset"),("@@IC:gear@@","Mécanique","sourcing + budget + annonces"),("@@IC:rocket@@","Accélération","KPIs + scaling"),("@@IC:check@@","Passage à l'action","checklist + plans datés")]) +
  ...
  process("Le cycle d'une vente, de A à Z", [
    ("@@IC:search@@","Trouver l'article"),
    ("@@IC:camera@@","Faire les photos"),
    ("@@IC:upload@@","Publier l'annonce"),
    ("@@IC:cart@@","Vendre"),
    ("@@IC:box@@","Emballer le colis"),
    ("@@IC:truck@@","Déposer le colis"),
  ]) +
  ...
  recap([
    ...
    "Vendre est facile&nbsp;; <span class=hlx>sourcer</span> est le vrai métier — et c'est le problème que ce guide va résoudre pour toi.",
...
page(
  ...
  box("error",
    '<p><strong>Confondre marge brute et marge nette.</strong> Beaucoup de débutants se réjouissent d\'un «&nbsp;acheté 8&nbsp;€, vendu 24&nbsp;€&nbsp;» sans compter les frais d\'expédition, les consommables et le temps. On raisonne <span class=hlx>toujours en marge nette</span>&nbsp;: c\'est le seul chiffre qui finit sur ton compte.</p>') +
  box("tip",
  ...
  recap([
    ...
    "Le trio qui décide de ta rentabilité&nbsp;: <span class=hlx>prix de revient, marge nette, rotation.</span>",
...
page(
  ...
  table(["Croyance de débutant @@IC:xmark@@","Croyance de pro @@IC:check@@"],
...
page(
  ...
  box("case",
    ...
    '<p>Sarah vend «&nbsp;un peu de tout&nbsp;» pendant deux mois&nbsp;: 60&nbsp;€, beaucoup de découragement. Elle décide de se <span class=hlx>concentrer sur une seule niche</span> (vestes en jean de marque) et d\'appliquer une règle&nbsp;: 5 photos soignées minimum par pièce. Résultat le mois suivant&nbsp;: <strong>230&nbsp;€ de ventes</strong>, des favoris qui grimpent, et surtout la sensation de <em>savoir ce qu\'elle fait</em>. «&nbsp;J\'ai arrêté de deviner, j\'ai commencé à piloter.&nbsp;»</p>') +
  box("error",
...
page(
  ...
  '<p class="lead">Un agent de sourcing, c\'est ton <span class=hlx>acheteur professionnel personnel</span>. Là où un simple fournisseur vend ce qu\'il a en rayon, l\'agent travaille <em>pour toi</em> sur les 4 tâches les plus chronophages du métier&nbsp;:</p>' +
  cards([
  ...
  '<p style="margin-top:2mm">En clair&nbsp;: l\'agent transforme la partie la plus difficile, la plus longue et la plus risquée du métier en un <span class=hlx>simple passage de commande</span>.</p>' +
  quote("Si tu veux aller vite, va seul. Si tu veux aller loin, va accompagné.","Proverbe africain") +
  box("tip",
    '<p>Même avec le meilleur agent, commence par une <span class=hlx>commande test</span>&nbsp;: un petit lot pour valider la qualité, tes photos, ton positionnement prix et ta rotation. Une fois le cycle validé (acheté → publié → vendu → réinvesti), tu montes les volumes en confiance. C\'est exactement le parcours que propose l\'agent Cheapsappes, du starter pack au réassort.</p>') +
  foot()
)
...
page(
  ...
  cards([
    ("@@IC:clock@@ Le temps","Friperies et brocantes&nbsp;: 4 à 6&nbsp;h pour 5-10 pièces. Un loisir sympa, pas un système. L'agent te rend ces heures pour ce qui vend&nbsp;: annonces, photos, clients."),
    ("@@IC:refresh@@ La régularité","Le vrai défi n'est pas de trouver une pièce, mais 30 par mois, tous les mois. La chine est une loterie&nbsp;; un business a besoin d'un flux."),
    ("@@IC:tag@@ Le prix","Seul, tu paies au prix du particulier. En volume via un agent, tu accèdes à des tarifs pros — et la marge se fait à l'achat."),
  ], cols=3) +
...
page(
  ...
  '<div class="chclose">' + box("ex",
    '<p>Calcule ton <span class=hlx>vrai taux horaire</span> de sourcing solo&nbsp;:</p>'
...
page(
  ...
  table(["","@@DOT:#12A06B@@ Starter — 100&nbsp;€","@@DOT:#1B63F0@@ Ambitieux — 300&nbsp;€","@@DOT:#7A3FF0@@ Accéléré — 500&nbsp;€"],
...
page(
  ...
  '<p class="lead">Le secret n\'est pas la mise de départ, c\'est <span class=hlx>de tout réinvestir</span> pendant les premiers mois. Voici la trajectoire d\'un capital de 100&nbsp;€ réinvesti (scénario prudent)&nbsp;:</p>' +
...
page(
  ...
  table(["Palier","Rythme","Marge nette / mois","Ce que ça change"],
    [["@@IC:target@@ Découverte","3-5 ventes / semaine","150-300&nbsp;€","Tu prouves que la méthode marche."],
     ["@@IC:rocket@@ Confirmation","1-2 ventes / jour","400-700&nbsp;€","Un vrai complément de revenu."],
     ["@@IC:flame@@ Accélération","3-5 ventes / jour","900-1&nbsp;500&nbsp;€","Un second salaire, un système qui tourne."]]) +
  box("case",
...
page(
  ...
  '<p><span class=hlx>Total&nbsp;: 30 à 50&nbsp;€</span>, une seule fois. Tout le reste, c\'est de la méthode.</p>' +
...
page(
  ...
  '<div class="grid g2" style="margin-top:1mm">'
  '<div class="card" style="border-left:3px solid #1B63F0"><div class="ct">@@IC:camera@@ Appli photo</div><p>Un outil de retouche simple (luminosité, recadrage) — pas de filtres trompeurs. La photo doit être fidèle.</p></div>'
  '<div class="card" style="border-left:3px solid #1B63F0"><div class="ct">@@IC:chart@@ Tableur</div><p>Ton tableau de bord&nbsp;: stock, coûts, ventes, marges. Deux onglets suffisent (voir chapitre 7).</p></div>'
  '<div class="card" style="border-left:3px solid #1B63F0"><div class="ct">@@IC:edit@@ Notes / modèles</div><p>Tes descriptions «&nbsp;à trous&nbsp;» et réponses types, pour publier une annonce en 4 minutes.</p></div>'
  '<div class="card" style="border-left:3px solid #1B63F0"><div class="ct">@@IC:wallet@@ Sous-compte bancaire</div><p>Sépare l\'argent du business de ton argent perso. Clarté comptable et mentale.</p></div>'
  '</div>' +
  hsub("La chaîne de production, en 4 gestes") +
  flow([("@@IC:basket@@","Trier","3 bacs"),("@@IC:wind@@","Défroisser","par lots"),("@@IC:camera@@","Photographier","5 photos/pièce"),("@@IC:upload@@","Publier","au créneau d'or")]) +
  box("case",
    '<div class="who">Le déclic organisation de Kevin — 26 ans, Nantes</div>'
    '<p>Kevin photographiait une pièce, la publiait, en prenait une autre… 20 minutes par annonce. En passant au <span class=hlx>travail par lots</span> (défroisser 15 pièces, puis les shooter à la chaîne, puis tout publier le dimanche), il est descendu à <strong>4 minutes par annonce</strong>. Même temps de travail, 3&nbsp;fois plus d\'annonces en ligne.</p>') +
  box("error",
    '<p><span class=hlx>Surinvestir avant la première vente.</span> Ring light, imprimante d’étiquettes, logiciels payants… s’équiper «&nbsp;comme un pro&nbsp;» avant d’avoir vendu une seule pièce transforme un business rentable en passe-temps coûteux. Ton smartphone et la lumière du jour suffisent pour tes 50 premières ventes.</p>') +
  box("tip",
    '<p>Monte une <span class=hlx>station de production permanente</span>&nbsp;: un coin fixe avec ton fond, ta lumière et tes 3 bacs, opérationnel en 30 secondes. Le frein n°1 à la publication, c’est le temps d’installation&nbsp;; en le supprimant, tu publies bien plus souvent — exactement comme la communauté Cheapsappes qui reçoit un stock déjà trié, prêt à photographier.</p>', tail=True) +
  hsub("Setup malin vs surinvestissement") +
  vs("Le piège&nbsp;: surinvestir",
     ["Ring light, imprimante, logiciels payants avant la 1ʳᵉ vente",
      "Du matériel qui dort, une trésorerie bloquée",
      "On s’achète l’illusion d’avancer"],
     "Le réflexe malin (&lt; 50&nbsp;€)",
     ["Smartphone + lumière du jour + fond neutre",
      "Défroisseur, cintres, enveloppes&nbsp;: l’essentiel",
      "On investit dans le stock, pas dans le décor"]) +
  '<div class="chclose">' + box("ex",
  ...
  recap([
    ...
    "Le vrai gain de temps vient du <span class=hlx>travail par lots</span>, pas d'outils coûteux.",
...
page(
  ...
  box("tip",
    '<p>La <span class=hlx>première photo</span> est ta vitrine dans le flux&nbsp;: cadrage serré, lumière naturelle, pièce nette. Les 4 suivantes rassurent&nbsp;; la première, elle, <span class=hlx>vend le clic</span>.</p>') +
  foot()
)
...
page(
  ...
  box("case",
    ...
    '<p>Inès avait 15 annonces «&nbsp;mortes&nbsp;» depuis des semaines. Un dimanche, elle refait <span class=hlx>seulement les photos et les titres</span> — sans toucher aux prix. Dans les 5 jours&nbsp;: <strong>6 ventes</strong>. Même stock, même prix, autre présentation. «&nbsp;Je croyais que mes pièces ne valaient rien. En fait, mes annonces ne les vendaient pas.&nbsp;»</p>') +
  box("error",
...
page(
  ...
  flow([("@@IC:refresh@@","Réassort auto","commande récurrente"),("@@IC:box@@","Stock trié","prêt à shooter"),("@@IC:camera@@","Batch photo","1 session/sem."),("@@IC:trend@@","Volume","ventes en hausse")]) +
  box("case",
    '<div class="who">Le passage à l’échelle de Dylan — 29 ans, Genève</div>'
    '<p>Dylan plafonnait à 15 ventes/mois&nbsp;: tout son temps partait en sourcing. Il met en place un <span class=hlx>réassort régulier via un agent</span> et bloque 4 créneaux fixes par semaine. Trois mois plus tard&nbsp;: <strong>2 à 3 ventes par jour</strong>, un dressing de 90 annonces, et… le même nombre d’heures qu’avant. «&nbsp;J’ai arrêté de chercher du stock. J’ai commencé à vendre.&nbsp;»') +
  box("error",
    '<p><span class=hlx>Scaler le chaos.</span> Amplifier un business désorganisé ne fait qu’amplifier le désordre&nbsp;: ruptures, retards d’envoi, mauvaises notes. Systématise <em>avant</em> d’accélérer&nbsp;: routine, tableau de bord et approvisionnement fiable d’abord.</p>') +
  box("tip",
    '<p>La contrainte n°1 du scaling, c’est presque toujours le stock. Sécurise un <span class=hlx>réassort automatique</span> (c’est le rôle de l’agent Cheapsappes) avant d’augmenter tes volumes de publication&nbsp;: tu ne veux jamais que tes meilleures annonces pointent vers un dressing vide.</p>', tail=True) +
  hsub("Ajouter des heures vs ajouter du système") +
  vs("Ajouter des heures",
     ["Travailler plus dur, au coup par coup",
      "Sourcing manuel qui plafonne le volume",
      "Tu restes le goulot d’étranglement"],
     "Ajouter du système",
     ["Travailler par lots, sur des créneaux fixes",
      "Réassort automatique via un agent",
      "Le système tourne pendant que tu vends"]) +
  '<div class="chclose">' + box("ex",
...
page(
  ...
  '<p class="lead">L’agent premium Cheapsappes, c’est l’accès direct à notre <span class=hlx>réseau de sourcing professionnel</span> — celui que nous avons mis des années à construire et à fiabiliser. En pratique&nbsp;:</p>' +
  benefits([
...
page(
  ...
  table(["Pack","Idéal pour","Contenu type","Objectif"],
    [["@@DOT:#12A06B@@ Starter","Valider ton cycle","~ un petit lot sélectionné","Prouver acheté → vendu → réinvesti"],
     ["@@DOT:#1B63F0@@ Croissance","Alimenter un dressing actif","Lot moyen, marques variées","Tenir 1-2 ventes/jour sans rupture"],
     ["@@DOT:#7A3FF0@@ Volume","Scaler sérieusement","Gros lot, réassort régulier","Soutenir plusieurs ventes/jour"]]) +
  ...
  '<div class="card" style="border-left:3px solid #E24435"><div class="ct">@@IC:xmark@@ Pas pour toi si…</div><p>tu cherches «&nbsp;de l’argent facile sans rien faire&nbsp;» (ça n’existe pas), ou si tu préfères la chine comme loisir du dimanche — ce qui est parfaitement respectable.</p></div>'
  '</div>' +
...
page(
  ...
  box("tip",
    '<p>Commence <span class=hlx>petit mais vite</span>&nbsp;: un starter pack aujourd’hui vaut mieux qu’un «&nbsp;gros lancement&nbsp;» dans trois mois. Le délai de livraison de ta première commande devient ton compte à rebours&nbsp;: il t’oblige à préparer ton setup et tes annonces pendant que le stock arrive.</p>') +
  box("error",
...
page(
  ...
  box("error",
    '<p><span class=hlx>Vouloir tout faire en un jour.</span> Le plan est réparti sur 7 jours pour une raison&nbsp;: chaque étape prépare la suivante. Brûler les étapes, c’est publier des annonces bâclées — et repartir de zéro. La régularité bat l’intensité.</p>', tail=True) +
  '<div class="chclose">' + box("ex",
...
page(
  ...
  '<p>Il y a six mois, ces vendeurs n’en savaient pas plus que toi. La seule différence entre eux et la version de toi qui lira encore des guides dans un an, c’est ce que tu fais dans les <span class=hlx>dix prochaines minutes</span>. Pas demain. Maintenant, pendant que la motivation est chaude.</p>' +
